...
def smooth_transitions_savgol(new_values, window_size=3, poly_order=1):
    """Smoothens transitions using the Savitzky–Golay filter."""
    smoothed_values = savgol_filter(new_values, window_size, poly_order)
...
def normalize_amplitudes(amplitudes, audio_data, previous_normalized=None, max_rows=6, alpha=0.5, window_size=5, poly_order=4):
    ...
    # Find the maximum amplitude of the audio data
    ...
    # Normalize the amplitudes to values between 0 and max_rows
    ...
    # If previous normalized values exist, apply smoothing
    if previous_normalized is not None:
        normalized = smooth_transitions_savgol(normalized, window_size=window_size, poly_order=poly_order)
    return normalized
